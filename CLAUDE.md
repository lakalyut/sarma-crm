...
специфичности) — плюс тумблер «Линия/Гистограмма» (`type` графика). Вся эта
логика — чисто на клиенте, без повторных походов на сервер, та же идея, что
...
конфликтах специфичности — на этом ловили баг дважды: `.is-hidden` из `base.css`
проигрывал и `.filter-tag` из `filters.css`, и `.regions-chart-split` из
`dashboard.css`, подключённым позже — при равной специфичности выигрывает тот, что
ниже в `_styles.html`; лечится либо переносом правила раньше, либо составным
селектором вида `.regions-chart-split.is-hidden`). Общий JS-паттерн поиска-с-подсказками
...
[checkbox_multiselect.js](app/static/js/checkbox_multiselect.js), который используется
везде, где есть фильтр периода — умеет схлопывать подряд идущие месяцы в диапазон
через флаг `dateRange: true` в конфиге; `admin/imports_delete.html` держит
почти идентичную локальную копию этой же функции, потому что месяцы туда
подгружаются через отдельный AJAX-эндпоинт, а не рендерятся Jinja сразу).
