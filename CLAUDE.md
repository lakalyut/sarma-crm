...
везде, где есть фильтр периода). При `dateRange: true` закрытый вид схлопывается: ≤3
месяцев — список через запятую, >3 подряд идущих — диапазон «месяц – месяц», >3 вразнобой —
«первые 3, +N» (тот же формат `+N`, что и в обычных мультиселектах без `dateRange`, только
там порог 2, а не 3 — пороги осознанно разные, не унифицированы). `admin/imports_delete.html`
держит почти идентичную локальную копию этой же функции, потому что месяцы туда
подгружаются через отдельный AJAX-эндпоинт, а не рендерятся Jinja сразу — правки логики
сворачивания нужно вносить в обоих местах. Рядом на страницах с фильтром периода есть ещё
текстовая подпись «Месяцы: ...» под фильтрами (`.selected-city-text`, на Клиентах/Графиках/
Амбассадорском отчёте/в шапке детализации клиента) — server-rendered, изначально не была
синхронизирована с этой логикой (всегда показывала полный список); теперь использует тот же
алгоритм через Jinja-фильтр `format_month_list` (`app/templating.py`) — контигуозность там
определяется по календарю (`next_month()` на `datetime.fromisoformat`), а не по позиции в
DOM, как в JS.

**`format_month_list`/`next_month()` не валидируют формат `Sale.month`** — импорт не
гарантирует `YYYY-MM-01` на старых/грязных данных (`get_months()` отдаёт что угодно
непустое), без `try/except` вокруг разбора дат это уже роняло страницу 500-й при выборе
региона; сейчас обёрнуто (падает в ветку «не непрерывно» вместо краша), но если увидите
похожий exception на новом месте — проверяйте именно этот угол первым.

**Статус на 2026-07-26 (открытый вопрос, не закрыт): после деплоя пользователь сообщил,
что фильтр по месяцам «работает криво» при выборе региона.** Конкретный сценарий не
уточнён, на синтетических локальных данных (в т.ч. намеренно испорченное значение месяца)
воспроизвести не вышло — только гипотетический краш-баг выше, который закрыт. Начинать
следующую сессию с уточнения репро у пользователя или прод-логов (`docker compose logs web`
на VPS), прежде чем править вслепую — см. ROADMAP.md.
